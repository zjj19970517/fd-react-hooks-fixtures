...
import { isDef } from '../../../common/utils/base'
import { formatNumber } from '../../../common/utils/number'
import { resetScroll } from '../../../common/utils/dom/scroll'
...
const Input = forwardRef<InputInstance, InputProps>((p, ref) => {
  ...
  const inputRef = useRef<HTMLInputElement>(null) // input 元素
  ...
  }, [props.clearTrigger, inputFocus, props.clearable, readOnly, value])
  ...
  // 失焦事件处理
  function handleBlur(event: React.FocusEvent<HTMLInputElement, HTMLInputElement>) {
    setInputFocus(false)
    props.onBlur?.(event)
    resetScroll()
  }
  ...
  // 聚焦事件处理
  function handleFocus(event: React.FocusEvent<HTMLInputElement, HTMLInputElement>) {
    setInputFocus(true)
    props.onFocus?.(event)
    if (readOnly) {
      blur()
  ...
  // 清除事件处理
  function handleClear(e: React.MouseEvent<HTMLDivElement, MouseEvent>) {
    setValue('')
    props.onClear?.(e)
  }
